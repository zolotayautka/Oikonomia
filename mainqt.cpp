    ...
    , ui(new Ui::mainQT)
{
    ...
    if (!(fileExists(QDir::homePath().toStdString() + "/.account_book"))){
        create_book();
    }
...
void mainQT::search(){
    ...
    for(int i = 0; i < l; i++){  // QTableWidgetItem客体はQTableWidget客体によって管理されるから別にdeleteを使う必要はありません。
        ...
        QTableWidgetItem* atai;
        if (!loc.compare("ja_JP")){
            atai = new QTableWidgetItem(QString::number(slist[i].atai) + "¥");
        } else {
            atai = new QTableWidgetItem(QString::number(slist[i].atai) + "₩");
        }
        ui->all_table->setItem(l-1-i, 3, atai);
        QTableWidgetItem* nokor;
        if (!loc.compare("ja_JP")){
            nokor = new QTableWidgetItem(QString::number(nokori) + "¥");
        } else {
            nokor = new QTableWidgetItem(QString::number(nokori) + "₩");
        }
...
void mainQT::load_sel_day(){  // QTableWidgetItem客体はQTableWidget客体によって管理されるから別にdeleteを使う必要はありません。
    ...
    for(int i = 0; i < l; i++){
        ...
        QTableWidgetItem* atai;
        if (!loc.compare("ja_JP")){
            atai = new QTableWidgetItem(QString::number(sel_list[i].atai) + "¥");
        } else {
            atai = new QTableWidgetItem(QString::number(sel_list[i].atai) + "₩");
        }
...
void mainQT::import_(){
    QMessageBox::StandardButton f;
    if(!loc.compare("ja_JP")){
        f = QMessageBox::question(this, "お知らせ", "既存のデータを維持しますか？",
                                        QMessageBox::Yes | QMessageBox::No);
    } else {
        f = QMessageBox::question(this, "알림", "기존 데이터를 유지하겠습니까?",
                                  QMessageBox::Yes | QMessageBox::No);
    }
    QString s = QCoreApplication::applicationDirPath();
    std::string path = QFileDialog::getOpenFileName(this, "Import CSV File", s, "CSV files (*.csv)").toStdString();
    if (f == QMessageBox::No){
        remove(".account_book");
        create_book();
    }
...
void mainQT::out_(){
    QString s = QCoreApplication::applicationDirPath();
    std::string path = QFileDialog::getSaveFileName(this, "Export CSV File", s, "CSV files (*.csv)").toStdString();
    out(path);
}
